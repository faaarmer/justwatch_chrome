...
class JustWatchChrome {
  ...
  execute() {
    if (
      document.querySelectorAll('*[itemtype="http://schema.org/TVSeries"]')
        .length > 0
    ) {
      this.type = "show";
    } else if (
      document.querySelectorAll('*[itemtype="http://schema.org/Movie"]')
        .length > 0
    ) {
      this.type = "movie";
    }

    this.ldJSON = document.querySelectorAll(
    ...
    for (let hostname in supportedWeb) {
      ...
      if (location.href.match(hostname)) {
        ...
        this.titleFull =
          this.title == null ? supportedWeb[hostname].title() : this.title;
  ...
  getPanelTitleHTML() {
    return (
      '<span id="justwatch-title" class="title">JustWatch:' +
      (debug ? "[" + l18n + "]" : "") +
      ' <span id="justwatch-title-full">' +
      this.titleFull +
      "</span></span>"
    );
  }
  ...
  setPanelTitleURL(item) {
    ...
    document.getElementById("justwatch-title-full").innerHTML =
      item.title + " (" + item.original_release_year + ")";
  }
  ...
  setCheapestOffer(offer, offersDiv, offersData) {
    ...
    if (typeof offer.retail_price != "undefined") {
      offersData[offer.monetization_type][offer.provider_id]["cheapest_price"] =
        offer.retail_price;
    } else {
      offersData[offer.monetization_type][offer.provider_id] = true;
    }

    var old = offersDiv.querySelectorAll(
      ".monetization-" +
  ...
  getOffersHTML(offers) {
    ...
    if (typeof offers !== "undefined" && offers.length > 0) {
      for (const [index, offer] of offers.entries()) {
        ...
        if (
          typeof offer.urls !== "undefined" /*offer.urls.length > 0*/ &&
          typeof offer.urls.standard_web !== "undefined"
        ) {
          domainString = offer.urls.standard_web
            .replace("http://", "")
            .replace("https://", "")
            .replace("www.", "")
            .split(/[/?#]/)[0];
        ...
        switch (offer.monetization_type) {
          case "flatrate":
            ...
            ulBlocks[offer.monetization_type].insertAdjacentHTML(
              "beforeend",
              '<li id="offer-' +
                index +
                '" ' +
          ...
          default:
            ulBlocks["other"].insertAdjacentHTML(
              "beforeend",
              '<li id="offer-' +
                index +
                '" ' +
                ...
                offer.presentation_type +
                '</span>  <span class="price">' +
                (typeof offer.retail_price !== "undefined"
                  ? offer.retail_price
                  : "0") +
                "" +
                (typeof offer.currency !== "undefined"
                  ? typeof currency[offer.currency] !== "undefined"
                    ? currency[offer.currency]
                    : offer.currency
                  : "-") +
                "</span></a></li>\n"
      ...
      for (var block in ulBlocks) {
        if (ulBlocks.hasOwnProperty(block)) {
          ...
          if (
            typeof offerBlock.childNodes == "undefined" ||
        ...
        if (typeof supportedWeb[hostname].waitForSelector != "undefined") {
          if (debug)
            console.log(
              hostname +
                ": waiting for [" +
                supportedWeb[hostname].waitForSelector +
                "]"
            );
